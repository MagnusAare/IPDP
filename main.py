...
def dataLoad(filename):
    ...
    conditions = [(10 > dataRaw[:, 0]) | (dataRaw[:, 0] < 60),  # Temperature
    ...
    for i in range(3):
        print(f'{errorTypes[i]} errors found and removed at indexes: {np.where(np.invert(conditions[i]))[0]}')
...
def dataStatistics(data, statistic):
    statDict = {'Mean Temperature': np.mean(data[:, 0]),
