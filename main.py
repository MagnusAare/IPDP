...
def dataLoad(filename: str) -> np.ndarray:
    ...
    conditions = [
        ...
        (dataRaw[:, 2] == 1) | (dataRaw[:, 2] == 2) | (dataRaw[:, 2] == 3) | (dataRaw[:, 2] == 4),  # Category
    ]
...
def dataStatistics(data: np.ndarray, statistic: str) -> float:
    """Calculates chosen statistic, the following options are included:
...
def dataPlot(data : np.ndarray):
    ...
    Output: Two plots.
    """
    names = np.array(['Salmonella', 'Bacillus cereus', 'Listeria', 'Brochothrix\nthermosphacta'])
    counts = np.zeros(4)
    ...
    for i in range(len(counts)):
        ax.scatter(data[data[:, 2] == i + 1][:, 0], data[data[:, 2] == i + 1][:, 1], color=color[i])
...
def dataFilter(data):
    """Filters the data on user input. Possible to filter on:\n 
        Temperature\n
        Growth rate\n
        Bacteria Type"""
    
    #Initialsation of the parameters
    done = False
    isValidTempLim = False
    isValidGrowthLim = False
    isValidCatLim = False
    filter = np.array([None] * 3)
    limitExplainer = f'All data outside these limits will not be included in the dataset\n' \
    ...
    while not done:
        filterPrompt = 'Please choose which attribute you want to filter \n' \
                       'a. Temperature\n' \
                       'b. Growth rate\n' \
                       'c. Bacteria Type\n' \
        ...
        if f == "a":
            tempPrompt = 'Please enter your desired limits for temperature as two (2) numbers seperated by ","\n' \
                  '(Example: "10, 60")'
            print(tempPrompt)
            while not isValidTempLim:
                try:
                    ...
                    print(f'Temperature limits have been set to: {filter[0]}\n'+limitExplainer)
                    isValidTempLim = True
        ...
        if f == "b":
            growthPrompt = 'Please enter your desired limits for growth rate as two (2) non-negative numbers seperated by ","\n' \
                  '(Example: "0, 1")'
            print(growthPrompt)
            while not isValidGrowthLim:
                try:
                    ...
                    print(f'Growth rate limits have been set to: {filter[1]}\n'+limitExplainer)
                    isValidGrowthLim = True
                except:
                    print('Not a valid input! >:(')
                    print(growthPrompt)
        #Filter for bacteria types
        if f == 'c':
            catPrompt = 'Please enter your desired bacteria types to include as numbers seperated by "," corresponding' \
            ...
            while not isValidCatLim:
                ...
                try:
                    ...
                    if lim[(lim != 1) & (lim != 2) & (lim != 3) & (lim != 4)].shape[0] != 0:
                        raise Exception
                    for c in lim:
                        tempdata = np.append(tempdata,data[data[:,2] == c],axis=0)
                    filter[2] = f'Bacteria types: {lim}'
                    print(f'Bacteria types have been set to {lim}\n'+categoryExplainer)
                    data = tempdata
                    isValidCatLim = True
                except:
                    print('Not a valid input! >:(')
...
def main():
    """Prompts to interact with user."""
    #Initialisation of parameters for user promts
    choice = None
    data = None
    filter = None
    ...
    while choice != 'x':
        ...
        choice = input(prompt)
        #Load of data based on user promt
        if choice == '1':
            askedNicely = 0

            while data is None:
                try:
                    data = dataLoad(input('Please enter a valid filename'+' >:( ' * askedNicely + ":\n"))
                except Exception as e:
                    askedNicely = 1
                    print('That is not a valid filename!')
        #Appling filter on data based on user prompt
        elif choice == "2":
            if data is not None:
                data, filter = dataFilter(data)

            else:
                print('No data is available to be filtered \n'
                      'Please load data via command 1 before proceeding\n')
        #Appling statistiscal meassures on data based on user prompt. 
        elif choice == "3":
            if data is not None:
                done = False
                while not done:
                    statPrompt = 'Which statistic do you want to generate?\n' \
                ...
                print('No data is available to be filtered \n'
                      'Please load data via command 1 before proceeding\n')
        #Plotting of data with filters based on user prompts.
        elif choice == "4":
            if data is not None:
                dataPlot(data)
            else:
                print('No data is available to be filtered \n'
                      'Please load data via command 1 before proceeding\n')
        #Termination of program with goodbye message.
        elif choice != "x":
